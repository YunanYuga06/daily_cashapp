...
class EditAnggaranPage extends StatefulWidget {
  ...
  const EditAnggaranPage({super.key, required this.budget});
...
class _EditAnggaranPageState extends State<EditAnggaranPage> {
  ...
  Future<void> _fetchInitialDataAndFillForm() async {
    _totalController.text = widget.budget.amount.toString();
    _catatanController.text = '';
    _selectedDateRange = DateTimeRange(start: widget.budget.firstPeriod, end: widget.budget.lastPeriod);
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _selectedCategory = _categories.firstWhere((c) => c.id == widget.budget.category.id);
          if (widget.budget.asset != null) {
            try {
              _selectedAsset = _assets.firstWhere((a) => a.id == widget.budget.asset!.id);
            } catch (e) {
              _selectedAsset = null;
            }
          }
      ...
      child: ListView(
        ...
        children: [
          ...
          TextFormField(
            ...
            validator: (value) {
              if (value == null || value.isEmpty) return 'Total tidak boleh kosong';
              if (int.tryParse(value.replaceAll('.', '')) == null) return 'Masukkan angka yang valid';
              return null;
            },
          ),
  ...
  Widget _buildAssetDropdown() {
    return DropdownButtonFormField<AssetModel>(
      ...
      items: _assets.map((asset) {
        return DropdownMenuItem<AssetModel>(value: asset, child: Text(asset.assetName));
      }).toList(),
      onChanged: (newValue) => setState(() => _selectedAsset = newValue),
    );
  }
}